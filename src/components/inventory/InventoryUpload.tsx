...
export default function InventoryUpload({ orgId, userEmail, onComplete }: {
  ...
  async function processAll() {
    ...
    // ── Build velocity lookup from in-memory data (no per-ASIN DB fetch) ──
    ...
    for (const snap of enrichedSnapshots) {
      ...
      const planCalc = calcPlanning(
        { ...asinRec, asin: asinRec.asin as string },
  ...
  return (
    <div style={{ padding: 24, maxWidth: 800 }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text2)' }}>Upload your weekly Amazon reports. The app auto-detects each file type.</div>
      </div>
      ...
      <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, padding: '12px 16px', marginBottom: 20, fontSize: 12, color: 'var(--text2)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 20px' }}>
          <div><span style={{ color: hasFba ? 'var(--accent)' : 'var(--text3)' }}>{hasFba ? '✓' : '①'}</span>{' '}<strong>FBA Report</strong> — Reports → Fulfilment → All Inventory<div style={{ color: 'var(--text3)', paddingLeft: 16 }}>Sales velocity (7/30/60/90d) + inbound</div></div>
          <div><span style={{ color: hasManageFba ? 'var(--accent)' : 'var(--text3)' }}>{hasManageFba ? '✓' : '②'}</span>{' '}<strong>Manage FBA</strong> — Inventory → Manage FBA → Download<div style={{ color: 'var(--text3)', paddingLeft: 16 }}>Fulfillable quantities (most accurate)</div></div>
          <div><span style={{ color: hasAwd ? 'var(--accent)' : 'var(--text3)' }}>{hasAwd ? '✓' : '③'}</span>{' '}<strong>AWD Inventory</strong> — Inventory → AWD → Download<div style={{ color: 'var(--text3)', paddingLeft: 16 }}>AWD buffer stock + inbound</div></div>
          <div><span style={{ color: hasReserved ? 'var(--accent)' : 'var(--text3)' }}>{hasReserved ? '✓' : '④'}</span>{' '}<strong>Reserved Inventory</strong> — Reports → Fulfilment → Reserved<div style={{ color: 'var(--text3)', paddingLeft: 16 }}>Customer orders excluded from coverage</div></div>
        </div>
      </div>

      {files.length > 0 && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
          {[{ label: 'FBA Report', has: hasFba }, { label: 'Manage FBA', has: hasManageFba }, { label: 'AWD Inventory', has: hasAwd }, { label: 'Reserved', has: hasReserved }].map(item => (
            <div key={item.label} style={{ padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 500, background: item.has ? 'var(--accent-light)' : 'var(--surface2)', border: `1px solid ${item.has ? 'rgba(26,107,60,.3)' : 'var(--border)'}`, color: item.has ? 'var(--accent)' : 'var(--text3)' }}>
              {item.has ? '✓' : '○'} {item.label}
            </div>
          ))}
        </div>
      ...
      <div onDragOver={e => { e.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={onDrop}
        style={{ border: `2px dashed ${dragging ? 'var(--accent)' : 'var(--border2)'}`, borderRadius: 10, padding: '32px 20px', textAlign: 'center', background: dragging ? 'var(--accent-light)' : 'var(--surface2)', transition: 'all .15s', cursor: 'pointer', marginBottom: 16 }}
        onClick={() => document.getElementById('file-input')?.click()}>
        <input id="file-input" type="file" accept=".csv,.txt,.tsv" multiple style={{ display: 'none' }} onChange={e => e.target.files && handleFiles(e.target.files)} />
        <div style={{ fontSize: 32, marginBottom: 8 }}>📂</div>
        <div style={{ fontSize: 14, fontWeight: 500, color: dragging ? 'var(--accent)' : 'var(--text2)', marginBottom: 4 }}>Drop all 4 files here or click to browse</div>
      ...
      {files.length > 0 && (
        <div style={{ marginBottom: 20 }}>
          {files.map((f, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 7, marginBottom: 8 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 500 }}>{f.name}</div>
                <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'var(--mono)', marginTop: 2 }}>{f.rows} rows</div>
              </div>
              <select value={f.type} onChange={e => overrideType(i, e.target.value as UploadedFile['type'])}
      ...
      {result && (
        <div style={{ background: result.asins > 0 ? 'var(--accent-light)' : 'var(--orange-light)', border: `1px solid ${result.asins > 0 ? 'rgba(26,107,60,.3)' : 'rgba(192,107,0,.3)'}`, borderRadius: 8, padding: '12px 16px', marginBottom: 16, fontSize: 13 }}>
          {result.asins > 0 ? '✓ ' : '⚠ '}{result.message}
          {result.asins > 0 && <button className="btn-secondary" style={{ marginLeft: 16, fontSize: 12, padding: '4px 10px' }} onClick={onComplete}>View Dashboard →</button>}
        </div>
      )}

      <div style={{ display: 'flex', gap: 10 }}>
        <button className="btn-primary" onClick={processAll} disabled={processing || files.length === 0} style={{ opacity: processing || files.length === 0 ? 0.6 : 1 }}>
          {processing ? '⟳ Processing…' : `Process ${files.length} file${files.length !== 1 ? 's' : ''}`}
        </button>
        {files.length > 0 && <button className="btn-secondary" onClick={() => setFiles([])}>Clear all</button>}
      </div>
