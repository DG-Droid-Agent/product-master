...
function parseBulkFile(buffer: Buffer): { rows: any[]; portfolios: string[]; portfolioSummary: any[] } {
  ...
  if (!ws) throw new Error('Sheet "SP Search Term Report" not found. Available: ' + wb.SheetNames.join(', '))

  // Amazon bulk files use row 0 as the header row ("Product", "Campaign ID" etc.)
  // but some workbook formats only populate the first cell, so SheetJS sheet_to_json
  // returns rows with only a "Product" key. Detect and re-read with raw:false.
  let raw = XLSX.utils.sheet_to_json(ws, { defval: '' }) as any[]
  const firstRow = raw[0] as Record<string, any> | undefined
  const hasSearchTerm = firstRow && ('Customer Search Term' in firstRow || 'customer_search_term' in firstRow)
  if (!hasSearchTerm && ws['!ref']) {
    raw = XLSX.utils.sheet_to_json(ws, { defval: '', raw: false }) as any[]
  }
  raw = (raw as any[]).filter((r: any) => {
    const st = String(r['Customer Search Term'] ?? r['customer_search_term'] ?? '').trim()
    return st && st !== 'Customer Search Term'
  })
  ...
  for (const r of raw) {
    ...
    // Primary signal: does the search term LOOK like an ASIN / product target?
    //   - ASIN format: starts with B0 + 8 alphanumeric chars, or is a category expression
    // Secondary signal: ID columns (unreliable — Amazon sometimes omits Keyword ID
    //   even for real keyword rows in Broad/Phrase/Exact campaigns)
    //
    // Rule: term is a PT row ONLY IF it looks like an ASIN/category AND has PT ID but no KW ID
    // If the term is plain text (phrase, unigram etc) → always keyword, regardless of IDs
    ...
    const looksLikeAsin = /^B0[A-Z0-9]{8,}$/i.test(searchTerm.replace(/\s/g,'')) || searchTerm.toLowerCase().startsWith('category=') || searchTerm.toLowerCase().startsWith('asin=')
    const targetingType: 'keyword' | 'pt' = (ptIdValid && !kwIdValid && looksLikeAsin) ? 'pt' : 'keyword'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const safeJsonParse = (s: string | null, fallback: any) => { try { return s ? JSON.parse(s) : fallback } catch { return fallback } }
    const campaignNames   = safeJsonParse(formData.get('campaign_names') as string | null, []) as string[]
    const campaignTypes   = safeJsonParse(formData.get('campaign_types') as string | null, []) as string[]
